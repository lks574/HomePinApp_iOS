...
- 하루치 세션 정리: `docs/agents/daily.md`
...
- `hpi:daily`: `docs/agents/daily.md` (운영 워크플로, `hpi:auto` 체인에는 미포함)
...
hpi:daily   하루치 세션(커밋+트랜스크립트) 정리·문서 갱신
...
`Decision/`·`follow-ups.md`·`Daily/`·`next-task.md` 에서 이번 작업과 관련된 과거
맥락을 회수한다. 목적은 "탐색 품질이 결과 품질을 결정한다" — 이미 내린 결정·폐기한
...
`docs/wiki/`는 옵시디언 vault다. 코드 문서가 "사실/규칙"을 담는다면 위키는
"맥락/회고/결정"을 담는다. *(구조 작성 예정)*

- `docs/wiki/index.md` — MOC, 진입 허브
- `docs/wiki/notes-guide.md` — 명명·태그·frontmatter·콜아웃 규약
- `docs/wiki/Daily/` — 날짜별 메모
- `docs/wiki/Weekly/` — 주간 회고
- `docs/wiki/Decision/` — 비가역 결정 (ADR 스타일)
- `docs/wiki/Research/` — 외부 자료·번역 정리
- `docs/wiki/Idea/` — 임시 아이디어

모든 코드 문서·위키 노트는 YAML frontmatter(`aliases`, `tags`, `created`,
`updated`, `status`)를 갖는다. 태그 체계와 frontmatter 스키마는
...
데일리·주간 노트는 **에이전트가 큰 틀을 잡고 사용자가 세부를 보태는** 방식으로
작성한다. 에이전트가 작업 세션을 마무리할 때 해당 노트의 초안을 만들고, 사용자는
필요한 맥락을 보탠다.

| 노트 유형 | 작성 주체 | 갱신 시점 |
| --- | --- | --- |
| Daily | 에이전트 초안 → 사용자 보강 | 작업 세션 종료 시 |
| Weekly | 에이전트 초안 → 사용자 보강 | 주 단위 회고 (월요일 또는 금요일) |
| Decision | 에이전트 작성 | 비가역 결정(저장소·데이터·권한·동기화 등) 발생 시 즉시 |
| Research | 사용자 또는 에이전트 | 외부 자료 정리 필요 시 |
| Idea | 사용자 | 자유 |

명명·태그·콜아웃·템플릿은 `docs/wiki/notes-guide.md` 참고. 위키 노트도 코드
문서와 동일한 frontmatter 스키마를 사용한다.
