...
- 검색(항상): `Item.normalizedName` 부분 일치(물건) + `Recipe.title`·재료명 부분 일치(레시피)로 입력 즉시 결과를 **물건/레시피 섹션**으로 구분 표시. 물건 결과 탭 시 [[ItemEditor]] `edit` 모드로 진입하고, 레시피 결과 탭 시 시트를 닫고 레시피 탭 상세([[RecipeDetail]])로 push(`AppRouter.openRecipe`).
...
- 읽기: [[Recipe]] (`@Query(sort: \Recipe.title)`, 제목·재료명 정규화 키로 in-memory 필터).
...
- 잔여: 실기기 추론·한국어 품질 검증, 모델 다운로드 유도 UX, 유통기한·메모. (find/add 의도 자동판별은 검색-우선 통합 UX 로 대체·불필요.)
