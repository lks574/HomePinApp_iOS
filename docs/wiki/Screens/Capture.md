...
status: in-progress
screen-id: screen-01
related-tasks: [screen-01, screen-04, screen-12, screen-16, screen-20, screen-21, screen-22]
---
...
- **입력 어댑터(`screen-21`·`screen-22`)**: bulk 칩 staging 코어에 합류하는 두 입력 어댑터. 둘 다 결과를 칩 staging UI 로 합류시키고 insert 는 "추가" 버튼에서만(자동 저장 없음). 시트는 진입 모드(`CaptureSheet.InitialMode`)로 템플릿·영수증 모드로 열 수 있고, bulk 모드 `adapterRow` 에 두 어댑터 진입 버튼이 상시 노출된다.
  ...
  - **③ 영수증 OCR(`screen-22`)**: `ReceiptScanSheet`(iOS 만, bulk adapterRow "Scan receipt") → 카메라(가용 시, `UIImagePickerController.cameraAvailable` 게이팅)·`PhotosPicker` 로 영수증 한 장 → 온디바이스 Vision OCR(`ReceiptTextRecognizer`, `.accurate`+ko/en, Sendable·비-MainActor) → 보수적 규칙 필터(`ReceiptLineFilter`, 합계/날짜/사업자번호 제거·줄끝 가격 분리) → `ItemQuickAddParser.parse(multiline:)` 통과 → 칩 합류. 상태는 `ReceiptScanViewModel`(@MainActor @Observable, 단일 Task). **영수증 이미지 미저장, FoundationModels 비채택**. 추출 0건/실패 → 빈 staging + 안내 폴백. 카메라 권한 거부 graceful. macOS 는 PhotosPicker 만(카메라 `#if os(iOS)`). 결정: [[2026-06-18-영수증-OCR-Vision-규칙추출]].
...
- `[screen-01]`, `[screen-04]`, `[screen-12]`, `[screen-16]`, `[screen-20]`, `[screen-21]`, `[screen-22]` (docs/screen-implementation-tasks.md)
- 관련 결정: [[2026-06-12-네비게이션-UI구조]], [[2026-06-15-음성입력-STT-아키텍처]], [[2026-06-16-레시피-NL파서-ParsedRecipe]], [[2026-06-17-검색-랭킹-초성-편집거리]], [[2026-06-17-자연어-규칙-외부화]], [[2026-06-18-area-생략-캡처-허용-및-멀티-추가]], [[2026-06-18-스타터-템플릿-칩합류]], [[2026-06-18-영수증-OCR-Vision-규칙추출]]
...
- 물건 추가 저장은 `HomePinApp/Sources/Features/Items/ItemEditorView.swift` 와
  ...
  - 영수증 OCR(`screen-22`): `HomePinApp/Sources/Features/Capture/ReceiptScanSheet.swift`(카메라/PhotosPicker 진입) + `HomePinApp/Sources/Shared/OCR/`(`ReceiptTextRecognizer`·`ReceiptLineFilter`·`CameraImagePicker`(iOS)·`ReceiptScanViewModel`). 권한은 `Project.swift` sharedInfoPlist `NSCameraUsageDescription`·`NSPhotoLibraryUsageDescription`(en/ko `InfoPlist.xcstrings`).
