...
- 연속 입력(여러 개 추가, `screen-20`)으로 한 번에 여러 Item 을 insert 할 때도 단건과 같은 불변식(name→normalizedName 동기화, `spot?.area ?? area`)을 적용한다 — `ItemBulkAddModel.bulkInsert`. 멀티 추가의 이름 중복은 차단 없는 경고만(유니크 제약 검토는 별도, `docs/follow-ups.md`).
